...
setup() {
  export TEST_ROOT
  TEST_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/clai-release-install-test.XXXXXX")"
  export FIXTURE_DIR="$TEST_ROOT/fixture"
  export FIXTURE_TAG=v1.2.3
  export FIXTURE_ASSET=clai_1.2.3_linux_amd64.tar.gz
  ...
  cat > "$FIXTURE_DIR/package/clai" <<'EOF'
#!/bin/sh
echo "clai version 1.2.3"
EOF
  chmod +x "$FIXTURE_DIR/package/clai"
...
@test "release installer resolves, verifies, and installs the latest release" {
  ...
  [[ "$output" == *"Installed clai to $TEST_ROOT/bin/clai (clai version 1.2.3)."* ]]
  grep -q '/repos/merefield/clai/releases/latest$' "$CURL_LOG"
  grep -q '/releases/download/v1.2.3/clai_1.2.3_linux_amd64.tar.gz$' "$CURL_LOG"

  run "$TEST_ROOT/bin/clai" --version
  [ "$status" -eq 0 ]
  [ "$output" = "clai version 1.2.3" ]
}

@test "release installer supports an explicit version and Darwin ARM64" {
  export FAKE_UNAME_S=Darwin
  export FAKE_UNAME_M=arm64
  export FIXTURE_ASSET=clai_1.2.3_darwin_arm64.tar.gz
