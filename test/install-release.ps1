...
$testRoot = Join-Path ([IO.Path]::GetTempPath()) ("clai-release-installer-test-" + [Guid]::NewGuid().ToString("N"))
...
$assetName = "clai_1.2.3+build.5_windows_amd64.zip"
$releaseDirectory = Join-Path $serverRoot "merefield\clai\releases\download\$releaseTag"
$latestDirectory = Join-Path $serverRoot "repos\merefield\clai\releases"
$fixtureBinary = Join-Path $packageDirectory "clai.exe"
...
try {
    ...
    try { & go build -trimpath -ldflags "-X github.com/merefield/clai/internal/app.buildVersion=$releaseTag" -o $fixtureBinary ./cmd/clai }
...
'@
    ...
    for ($attempt = 0; $attempt -lt 50; $attempt++) {
        try { Invoke-WebRequest -Uri "$baseUrl/repos/merefield/clai/releases/latest" -UseBasicParsing | Out-Null; $ready = $true; break }
        catch { Start-Sleep -Milliseconds 100 }
    }
    if (-not $ready) { throw "fixture HTTP server did not start" }

    foreach ($name in @("CLAI_VERSION", "CLAI_REPOSITORY", "CLAI_GITHUB_URL", "CLAI_GITHUB_API_URL")) { $savedEnvironment[$name] = [Environment]::GetEnvironmentVariable($name) }
    $env:CLAI_VERSION = "latest"; $env:CLAI_REPOSITORY = "merefield/clai"; $env:CLAI_GITHUB_URL = $baseUrl; $env:CLAI_GITHUB_API_URL = $baseUrl
    ...
    $installedBinary = Join-Path $installDirectory "clai.exe"
    if ((& $installedBinary --version 2>&1 | Out-String).Trim() -ne "clai version $releaseVersion") { throw "installed binary reported an unexpected version" }
    ...
    New-Item -ItemType Directory -Path (Join-Path $directoryTarget "clai.exe") -Force | Out-Null
