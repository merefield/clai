...
repository=${CLAI_REPOSITORY:-merefield/clai}
release_tag=${CLAI_VERSION:-latest}
bin_dir=${CLAI_BIN_DIR:-/usr/local/bin}
binary_name=${CLAI_BIN_NAME:-clai}
github_url=${CLAI_GITHUB_URL:-https://github.com}
github_api_url=${CLAI_GITHUB_API_URL:-https://api.github.com}

usage() {
  cat <<'EOF'
Install a pre-built CLAI release from GitHub.
...
Environment:
  CLAI_VERSION         Release tag to install (default: latest).
  CLAI_BIN_DIR         Installation directory (default: /usr/local/bin).
  CLAI_BIN_NAME        Installed binary name (default: clai).
  CLAI_REPOSITORY      GitHub owner/repository (default: merefield/clai).
  CLAI_GITHUB_URL      GitHub web base URL (primarily for testing/mirrors).
  CLAI_GITHUB_API_URL  GitHub API base URL (primarily for testing/mirrors).
EOF
}

fail() {
  printf 'clai installer: %s\n' "$*" >&2
  exit 1
}
...
case "$repository" in
  /*|*/|*//*|*[!A-Za-z0-9._/-]*) fail "invalid CLAI_REPOSITORY: $repository" ;;
esac
repository_owner=${repository%%/*}
repository_name=${repository#*/}
if [ -z "$repository_owner" ] || [ -z "$repository_name" ] || [ "$repository_name" != "${repository_name#*/}" ]; then
  fail "CLAI_REPOSITORY must have the form owner/repository"
fi
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/clai-release-install.XXXXXX")
trap 'rm -rf "$tmp_dir"' EXIT HUP INT TERM

if [ "$release_tag" = latest ]; then
  printf 'Resolving the latest CLAI release...\n'
...
member_count=$(tar -tzf "$archive_path" | awk '$0 == "clai" { count++ } END { print count + 0 }')
[ "$member_count" -eq 1 ] || fail "release archive does not contain exactly one root-level clai binary"
extract_dir=${tmp_dir}/extract
mkdir "$extract_dir"
tar -xzf "$archive_path" -C "$extract_dir" clai
candidate=${extract_dir}/clai
[ -x "$candidate" ] || fail "the downloaded clai binary is not executable"

version_output=$("$candidate" --version 2>&1) || fail "the downloaded clai binary failed its version check"
[ "$version_output" = "clai version $release_version" ] ||
...
if [ -w "$bin_dir" ] || { [ ! -e "$bin_dir" ] && [ -w "$(dirname "$bin_dir")" ]; }; then
  ...
  command -v sudo >/dev/null 2>&1 || fail "$bin_dir is not writable and sudo is unavailable; set CLAI_BIN_DIR to a writable directory"
